...
// 
// Estudo de caso sobre aerogerador FEEL 900kW instalado na pedreira do Dalba em
// Guarapuava, com torre de 75m, considerando sistema de compensação de energia.
// Solicitado por Paulo Furlan.
//
// Data: 03/05/2013
//
// ----------------------------- PARÂMETROS ------------------------------------
...
projeto.vel_media = 5.736;              //Velocidade média anual de vento (m/s)
...
conta.tarifa_f_ponta = 310;             //Tarifa energia elétrica fora de ponta (R$/MWh)
conta.tarifa_demanda = 7.81;            //Tarifa demanda (R$/kW)
conta.tarifa_demanda_ultr = 15.63;      //Tarifa adicional demanda ultrapassada (R$/kW)
conta.consumo_ponta = 0;                //Consumo médio energia elétrica ponta (MWh)
conta.consumo_f_ponta = 146.268;        //Consumo médio energia elétrica fora de ponta (MWh)
conta.demanda_contratada = 900;         //Demanda contratada(kW)
