...
// 
// Estudo de caso genérico sobre aerogerador Northern 100-24, com torre de 37m,
// considerando sistema de compensação de energia. Solicitado por Paulo Furlan.
//
// Data: 06/05/2013
//
// ----------------------------- PARÂMETROS ------------------------------------
...
projeto.potencia = 0.1;                 //Potência instalada (MW)
projeto.vel_media = 5.5;                //Velocidade média anual de vento (m/s)
projeto.regime = 2;                     //Regime de produção de energia [1-Produção independente;
                                        //2-Sistema de compensação de energia; 3-Autoprodução]
projeto.preco_energia = 215;            //Preço da energia (R$/MWh)
...
projeto.custo_MW = 300e3/(0.1*0.7);     //Custo do MW instalado (R$/MW)
...
conta.consumo_f_ponta = 175;            //Consumo médio energia elétrica fora de ponta (MWh)
conta.demanda_contratada = 600;         //Demanda contratada(kW)
conta.demanda_ultr = 103.81;            //Demanda ultrapassada média (kW)

// Configuração da turbina:
turbina.diametro_rotor = 54;            //Diâmetro do rotor (m)
turbina.potencia_nominal = 100;         //Potência nominal do gerador (kW)
...
turbina.conceito = 4;                   //Conceito da turbina [1-Three-Stage Drive with High-Speed Generator;
                                        ...
                                        //4-Direct Drive]
turbina.modelo = 3;                     //Modelo da turbina [1-FEEl 900kW; 2-Gamesa G144; 3-Northern 100-24]
