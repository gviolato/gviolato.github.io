...
// 
// Estudo de caso para site da NOTUS, considerando turbina Gamesa G114. Fator de
...
projeto.potencia = 10*2.0;              //Potência instalada (MW)
projeto.vel_media = 7.5;                //Velocidade média anual de vento (m/s)
projeto.regime = 3;                     //Regime de produção de energia [1-Produção independente;
                                        //2-Sistema de compensação de energia; 3-Autoprodução]
projeto.preco_energia = 110;            //Preço da energia (R$/MWh)
...
projeto.fator_capacidade = 0.29;        //Fator de capacidade do parque eólico (MWh/ano)
...
conta.consumo_ponta = 80*4.1;           //Consumo médio energia elétrica ponta (MWh)
conta.consumo_f_ponta = 80*150.325;     //Consumo médio energia elétrica fora de ponta (MWh)
conta.demanda_contratada = 80*600;      //Demanda contratada(kW)
conta.demanda_ultr = 80*187.1;          //Demanda ultrapassada média (kW)

// Configuração da turbina:
turbina.diametro_rotor = 114;           //Diâmetro do rotor (m)
turbina.potencia_nominal = 2000;        //Potência nominal do gerador (kW)
turbina.altura_hub = 120;               //Altura do hub (m)
turbina.modelo_torque = 1;              //Modelo para determinação do máximo torque
...
turbina.conceito = 1;                   //Conceito da turbina [1-Three-Stage Drive with High-Speed Generator;
                                        ...
                                        //4-Direct Drive]
turbina.modelo = 2;                     //Modelo da turbina [1-FEEl 900kW; 2-Gamesa G114]
...
financiamento.TJLP = 0.035;              //Taxa nominal de juros de longo prazo - TJLP (a.a.)
financiamento.spread_basico = 0.00;    //Spread básico (a.a.)
financiamento.spread_risco = 0.0;      //Spread de risco (a.a.)
